...
from .models import Student, User, StudentGuardian, PrevAcademicRecord, Teacher
...
def enroll_student(modeladmin, request, queryset):
    ...
    for student in queryset:
        s = Student.objects.only('user').get(user__id=student.user.id)
...
class StudentGuardianAdmin(GuardedModelAdmin):
    list_display = ["guardian_name", "CNIC", "phone_number", "relationship", "occupation"]

class PrevAcademicRecordAdmin(GuardedModelAdmin):
